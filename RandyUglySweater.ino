...
//uint32_t yellow = strip.Color(254, 254, 51);
//uint32_t purple = strip.Color(191, 0, 255);
//uint32_t safetyOrange = strip.Color(255, 103, 0);
int red[]={255, 0, 0};
  ...
  switch (local_mode){
    case compass:
      ...
      if((lCurrentMillis>=0)&&(lCurrentMillis<=ModeMillis+30000)){
        ...
        theaterChase(lColor, theaterWait);
...
//          ModeMillis = lCurrentMillis;
      } else if((lCurrentMillis > ModeMillis+120001) && (lCurrentMillis<=ModeMillis+150000)){
        Serial.println("Fifth Mode");
         for(int m=0;m<5;m++) {
            CandyCane(red, white, 10,8,50);    //  This is a good one 10, 8, 50
         }
          ModeMillis = lCurrentMillis;
      } else if(lCurrentMillis > ModeMillis+150001){
          ModeMillis = lCurrentMillis;
...
void CandyCane(int *color1, int *color2, int sets,int width,int wait) {
  int L;
  uint32_t c1 = strip.Color(color1[0],color1[1],color1[2]);
  uint32_t c2 = strip.Color(color2[0],color2[1],color2[2]);
  
  for(int j=0;j<(sets*width);j++) {
    for(int i=0;i< strip.numPixels();i++) {
      L=strip.numPixels()-i-1;
      if ( ((i+j) % (width*2) )<width){
        strip.setPixelColor(L,c1);
      }else{
...
void fadeAllColors(int fade_speed, int wait) {
  ...
  for(int j=0;sizeof(colors);j++) {
    for(int i=0;i< 255;i+=fade_speed) {
      fadeValue = strip.Color(colors[j][0]/(255-i), colors[j][1]/(255-i), colors[j][2]/(255-i));
    ...
    for(int k=254;k> 0;k-=fade_speed) {
      fadeValue = strip.Color(colors[j][0]/(255-k), colors[j][1]/(255-k), colors[j][2]/(255-k));
      setAllColor(fadeValue);
      delay(wait);
    }
  }
